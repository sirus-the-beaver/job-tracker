  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        // If there are less than 3 recent applications, show all of them in reverse order
        // Otherwise, show the last 3 applications in reverse order
        response.data.length < 3 ? setRecentApps(response.data.reverse()) : setRecentApps(response.data.slice(response.data.length - 3, response.data.length).reverse());
      } catch (error) {
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="max-w-7xl mx-auto">
            <div className="mb-6">
              ...
              <p className="text-gray-600 mt-1 text-sm sm:text-base">Here's your job search summary</p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <div className="sm:hidden space-y-4">
                  {recentApps.length > 0 ? (
                    recentApps.map((app) => (
                      <div key={app.job_id} className="px-4 py-2 border-b border-gray-200">
                        <div className="flex justify-between items-center">
                          <div>
                            <h4 className="text-sm font-medium text-gray-900">{app.positionTitle}</h4>
                    ...
                    <div className="px-4 py-2 text-center text-sm text-gray-500">No recent applications found</div>
                  )}
                </div>
